...
if($(".panel .compact").length>0){
	...
	if($controller.length>0){
		...
		$controller.on("click",controllerSelector, function(){
			...
			$celular=$('.celular[data-slider-id="'+sliderId+'"]'),
			...
			if(!$controllerElement.hasClass("selected")){
				...
				$slider.children(".selected").removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend",
					function(){$(this).removeClass("hide");});
				$slider.children("li").eq(nextIndex).removeClass("hide").addClass("selected");

				$celular.children(".selected").removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend",
					function(){$(this).removeClass("hide");});
				$celular.children("li").eq(nextIndex).removeClass("hide").addClass("selected");

				$(".videoCell.showVideo")[0].pause();
				$celular.find("li.selected > video").removeClass("hideVideo").addClass("showVideo");
				$celular.find("li.hide > video").eq(nextIndex).removeClass("showVideo").addClass("hideVideo");
				$(".videoCell.showVideo")[0].play();				
	...
	$("[data-slider-action]").click(function(){
		if($(this).data("slider-id")){
			var $this=$(this),$desiredElement,nextIndex,sliderID=$this.data("slider-id"),action=$this.data("slider-action"),
			$slider=$('.slider[data-slider-id="'+sliderID+'"]'),
			...
			$selected=$slider.find(".selected"),isAnimated=$slider.hasClass("animated");
			...
			if(action==="next"){
				...
				if(action==="prev"){
					...
					if(parseInt(action)||action===0){nextIndex=parseInt(action);
